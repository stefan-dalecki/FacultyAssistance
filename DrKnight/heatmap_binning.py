...
file = r'C:\Users\stda9924\Uni\CU\Knight_Rotation\CSV_Files\Results\Quiz'+quiz_num+'_binning.csv'

df = pd.read_csv(file)

if 'no pre-grade for calculation' in df['Post Minus Pre']:
    df_postpre = df[~df['Post Minus Pre'].str.contains('no pre-grade for calcuation')]
else:
    df_postpre = df
...
def heatmap(df, col1, col2):
    ...
    value_counts = quiz_heatmap.groupby([col1, col2]).size().reset_index().rename(columns={0:'count'})
    df_array_style = pd.DataFrame(index = range(df[col2].min(), df[col2].max()+1), columns = range(df[col1].min(), df[col1].max()+1))
    ...
    if col2 == grade:
        for i in range(df[col2].min(), df[col2].max()+1):
            for j in range(df[col1].min(), df[col1].max()+1):
                ...
                text = ax.text(j, i, heat_array[i, j], ha="center", va="center", color=textcolors[int(im.norm(heat_array[i, j]) > threshold)])

    fig.tight_layout()
    plt.show(block = True)

heatmap(df, aware, grade)
