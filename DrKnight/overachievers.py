"""Investigating the overachieve students"""
import functools
import pandas as pd
import numpy as np
...
df = (
    functools.reduce(
        lambda left, right: pd.merge_ordered(left, right, on=["S_ID"], how="outer"),
        data_frames,
    )
    .fillna(np.nan)
    .dropna()
...
def changes(list: list) -> str:
    ...
    Args:
        list (list): list of grades

    Returns:
        str: improvement description
    """
